...
isStoredInMemory:		
	...
	addi $t8, $zero, 0	#t8 = count = 0
		...
		add $t5, $s2, $t2 	#t5 = StringList + i
		lb $t5, 0($t5)
		beq $t5, $zero, finishTraverseRecentStringList
		
		slti $t4, $t2, 1000
		beq $t4, $zero, finishTraverseRecentStringList
		addi $t3, $zero, 0	#t3 = j = 0
		
		compareString:	
			#So sánh kí tự của 2 string
			add $t6, $s2, $t2	#t6 = StringList + i
			add $t6, $t6, $t3	#t6 = t6 + j
			lb $t6, 0($t6)		#t6 = StringList[i+j]
			add $t7, $s0, $t3	#t7 = Input + j
			lb $t7, 0($t7)		#t7 = Input[j]
			bne $t6, $t7, notEqual	#Input[j]!=StringList[i+j]->not equal
			beq $t6, $zero, isEqual #Input[j]=StringList[i+j]='\0'->equal
			addi $t3, $t3, 1	#j++
			j	compareString
